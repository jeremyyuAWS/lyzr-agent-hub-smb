/*
  # Create agents table for SMB agent showcase

  1. New Tables
    - `agents_showcase`
      - `id` (uuid, primary key)
      - `name` (text, required) - Agent name
      - `description` (text, required) - Agent description
      - `category` (text, required) - sales, marketing, customer-service, hr, finance
      - `tags` (jsonb) - Array of tags for filtering
      - `demo_url` (text, optional) - Link to live demo
      - `status` (text) - live, coming_soon, optional
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Security
    - Enable RLS on `agents_showcase` table
    - Add policy for anonymous users to read all agents
    - Add policy for anonymous users to insert new agents
    - Add policy for anonymous users to update agents

  3. Data
    - Insert sample agents from existing data
*/
